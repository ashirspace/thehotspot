...
function LoginPage({ onLogin }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (records.length > 0) {
        ...
        const userData = { username: user.username, email: user.email, method: "password", role: user.role || "user" };
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      await airtableCreate({
        username, email, password, method: "password", role: "user",
  ...
  const handleGoogleLogin = () => {
    if (GOOGLE_LOGIN_CLIENT_ID === "YOUR_GOOGLE_CLIENT_ID_HERE") {
      ...
      popup.document.write(`<!DOCTYPE html><html><head><title>Google Sign-In</title></head><body style="font-family:-apple-system,sans-serif;background:#1a1a2e;color:#e0e0e8;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;margin:0;"><div style="background:#111118;border-radius:16px;padding:40px;text-align:center;max-width:340px;border:1px solid #2a2a3a;"><svg width="48" height="48" viewBox="0 0 48 48"><path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/><path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/><path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/><path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/></svg><h2 style="margin:20px 0 8px;font-size:18px;">Sign in with Google</h2><p style="color:#6b6b80;font-size:13px;margin-bottom:20px;">Use your Google account to access thehotspot</p><input id="em" type="email" placeholder="Enter your email" style="width:100%;padding:10px 14px;border-radius:8px;border:1px solid #2a2a3a;background:#0c0c12;color:#e0e0e8;font-size:13px;margin-bottom:12px;outline:none;box-sizing:border-box;" /><button onclick="var e=document.getElementById('em').value;if(e){window.opener.postMessage({type:'google-login',email:e},'*');window.close();}else{document.getElementById('em').style.borderColor='#f87171';}" style="background:#4285F4;color:#fff;border:none;padding:11px 28px;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;width:100%;font-family:sans-serif;">Continue</button></div></body></html>`);
    } else {
  ...
  useEffect(() => {
    const handler = async (e) => {
      if (e.data?.type === "google-login" && e.data?.email) {
        const gEmail = e.data.email;
        // Check if google user exists in Airtable
        const existing = await airtableFetch(`{email}='${gEmail}'`);
        if (existing.length === 0) {
          // Auto-create google user
          await airtableCreate({
            username: gEmail.split("@")[0], email: gEmail, password: "", method: "google", role: "user",
            created_at: new Date().toISOString().split("T")[0],
          });
        }
        const userData = { username: gEmail.split("@")[0], email: gEmail, method: "google", role: existing[0]?.fields?.role || "user" };
        localStorage.setItem("thehotspot_user", JSON.stringify(userData));
        onLogin(userData);
      }
    };
