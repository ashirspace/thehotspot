...
export function PricingPage() {
  return (
    <main className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      <h1 className="font-heading italic text-[clamp(3rem,6vw,5.5rem)] font-normal leading-[1] tracking-[-0.02em]">Pricing that scales with replies.</h1>
      <p className="mt-4 max-w-2xl text-[var(--text-secondary)]">Every plan includes workspace isolation, suppression lists, and safety-first sending controls.</p>
        ...
        {plans.map(({ name, price, desc, features }) => (
          <Card
            ...
            className={`relative rounded-none border-0 border-b border-[var(--surface-border)] p-8 lg:border-b-0 lg:border-r last:border-r-0 ${
              name === "Growth" ? "z-10 -my-3 border-2 !border-[var(--orange)] bg-[var(--surface-card)] py-11" : name === "Scale" ? "bg-[var(--surface-raised)]" : ""
            }`}
          >
            {name === "Growth" ? (
              <span className="absolute right-3 top-3 rotate-6 bg-[var(--orange)] px-2 py-1 font-mono text-[0.62rem] uppercase tracking-[0.12em] text-white">
                Most popular
              </span>
            ) : null}
