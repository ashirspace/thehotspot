...
export async function initDb(sql) {
  ...
  await sql`CREATE TABLE IF NOT EXISTS linkedin_campaigns (id SERIAL PRIMARY KEY, user_id TEXT, company TEXT, website TEXT, goal TEXT, offer_context TEXT, tone TEXT, status TEXT DEFAULT 'draft', draft_count INTEGER DEFAULT 0, approved_count INTEGER DEFAULT 0, sent_count INTEGER DEFAULT 0, skipped_count INTEGER DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW())`;
  await sql`CREATE TABLE IF NOT EXISTS linkedin_dm_drafts (id SERIAL PRIMARY KEY, campaign_id INTEGER, name TEXT, title TEXT, linkedin_url TEXT, department TEXT, seniority TEXT, notes TEXT, category TEXT, reason TEXT, message TEXT, approved BOOLEAN DEFAULT FALSE, status TEXT DEFAULT 'draft', created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW())`;
