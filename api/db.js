...
async function initDb(sql) {
  ...
  await sql`CREATE TABLE IF NOT EXISTS campaigns (id SERIAL PRIMARY KEY, user_id INTEGER, category TEXT, offer_context TEXT, sent_count INTEGER DEFAULT 0, failed_count INTEGER DEFAULT 0, cancelled BOOLEAN DEFAULT FALSE, status TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`;
  ...
  await sql`CREATE TABLE IF NOT EXISTS scheduled_campaigns (id SERIAL PRIMARY KEY, user_id INTEGER, scheduled_for TIMESTAMPTZ, category TEXT, offer_context TEXT, max_chars INTEGER, status TEXT DEFAULT 'pending', created_at TIMESTAMPTZ DEFAULT NOW())`;
